...
public class GameScreenTest extends Assertions {
    ...
    void testGetLaserViews(){
        //given
        List<LaserBeam> lasers = List.of(Mockito.mock(LaserBeam.class), Mockito.mock(LaserBeam.class), Mockito.mock(LaserBeam.class));
        Mockito.when(player.getLaserBeams()).thenReturn(lasers);
    ...
    void drawAstroids() throws IOException {
        //given
        List<AsteroidView> asteroids = List.of(Mockito.mock(AsteroidView.class), Mockito.mock(AsteroidView.class), Mockito.mock(AsteroidView.class));
        Mockito.doReturn(asteroids).when(screen).getAsteroidsView();
        //when
        screen.draw();
    ...
    void setGraphics(){
        ...
        for (var asteroid: asteroids )
            Mockito.verify(asteroid, Mockito.times(1)).setGraphics(graphics);
